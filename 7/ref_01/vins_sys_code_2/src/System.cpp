...
// 重载图像获取函数,直接获取特征点
void System::PubImageData(double dStampSec, Eigen::Matrix4d T_cw, string &feature_name)
{
...
//    }
    ...
    if(PUB_THIS_FRAME)
    {
        ...
        {
            std::string s;
            std::getline(f,s);//读取一行特征数据
            if(! s.empty())
            {
                std::stringstream ss;//将读取到的一行数据赋值给ss(x,y,z,1,u,v)
                ss<<s;
                ...
                Eigen::Vector4d temp = T_cw * point_3d;

                //cout << "after transform, point 3d coor :" << temp.transpose() << endl;

                if (temp(2) ==0)
                    break;
                double x = temp(0) / temp(2);
                double y = temp(1) / temp(2);
                double z = 1.0;
                ...
                feature_points->velocity_x_of_point.push_back(0);//时间戳不对齐的问题
                feature_points->velocity_y_of_point.push_back(0);
            }
            pts_id++;
            //cout << "pts_id :" << pts_id << endl;
        }
