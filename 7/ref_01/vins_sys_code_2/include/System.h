...
public:
    ...
    // 重载图像获取函数,直接获取特征点
    void PubImageData(double dStampSec, Eigen::Matrix4d T_cw, string &feature_name);
...
private:
    ...
    std::queue<ImuConstPtr> imu_buf;//队列,先进先出的一种数据结构
    std::queue<ImgConstPtr> feature_buf;
    // std::queue<PointCloudConstPtr> relo_buf;
    int sum_of_wait = 0;

    std::mutex m_buf;//mutex 类是能用于保护共享数据免受从多个线程同时访问(互斥)
    std::mutex m_state;
    std::mutex i_buf;
