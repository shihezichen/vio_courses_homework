...
iter: 0 , chi= 36048.3 , Lambda= 0.001
           saved currentLambda_: 0.001
           saved currentLambda_: 0.002
           saved currentLambda_: 0.008
           saved currentLambda_: 0.064
           saved currentLambda_: 1.024
           saved currentLambda_: 32.768
       saved currentLambda_: 2097.15
           iter: 1 , chi= 30015.5 , Lambda= 699.051
           saved currentLambda_: 699.051
           saved currentLambda_: 1398.1
           saved currentLambda_: 5592.41
       iter: 2 , chi= 13421.2 , Lambda= 1864.14
           saved currentLambda_: 1864.14
       iter: 3 , chi= 7273.96 , Lambda= 1242.76
           saved currentLambda_: 1242.76
       iter: 4 , chi= 269.255 , Lambda= 414.252
           saved currentLambda_: 414.252
       iter: 5 , chi= 105.473 , Lambda= 138.084
           saved currentLambda_: 138.084
       iter: 6 , chi= 100.845 , Lambda= 46.028
           saved currentLambda_: 46.028
       iter: 7 , chi= 95.9439 , Lambda= 15.3427
           saved currentLambda_: 15.3427
       iter: 8 , chi= 92.3017 , Lambda= 5.11423
           saved currentLambda_: 5.11423
       iter: 9 , chi= 91.442 , Lambda= 1.70474
           saved currentLambda_: 1.70474
       iter: 10 , chi= 91.3963 , Lambda= 0.568247
           saved currentLambda_: 0.568247
       iter: 11 , chi= 91.3959 , Lambda= 0.378832
         saved currentLambda_: 0.378832
    ...
    ![lambda随迭代曲线变化图](D:\Study\vio\3\images\lambda.png  "lambda随迭代曲线变化图")
  ...
  - 4. 输出如下, 可以看到优化后的参数和真实值差别很小, 在1.e-2级别:
    ```PowerShell
        ...
        iter: 1 , chi= 0.913952 , Lambda= 0.000333333
          saved currentLambda_: 0.000333333
        iter: 2 , chi= 0.91395 , Lambda= 0.000222222
          saved currentLambda_: 0.000222222
        problem solve cost: 0.429004 ms
           makeHessian cost: 0.18227 ms
        -------After optimization, we got these parameters :
    ...
    ```
- 5. 执行python脚本, lambda随迭代的变化图形如下:
![ax^2+bx+c的lambda输出变化](D:\Study\vio\3\images\new_lambda.png 'ax^2+bx+c的lambda输出变化')
...
- #### 目标3: 如果有实现其他阻尼因子更新策略可加分(选做).
  ...
  程序中其他参数：　int N = 700;   且 lambda 使用Hessian矩阵对角线上的最大值作为初始值
  ![四种lambda策略比较](D:\Study\vio\3\images\lambda_comparation.png "四种lambda策略比较")
...
由 $ \mathbf{J}^{\top} \mathbf{J} = \mathbf{V} \mathbf{\Lambda} \mathbf{V}^{\top} $ , 设 $\lambda_i$ 为特征值,  $ \mathbf{V}=(v_1, v_2, \cdots , v_n) $ 其中 $v_i$ 是列向量 ：
...
&= \mathbf{V}^{\top} diag \left( \frac {1}{\lambda_1 + \mu} \quad \frac {1}{\lambda_2 + \mu} \quad \cdots \quad  \frac {1}{\lambda_n + \mu} \right) \mathbf{V}  \left(- \mathbf{J}^{\top} \mathbf{f} \right) \\
