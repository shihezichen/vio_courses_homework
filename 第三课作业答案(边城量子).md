...
由 $ \mathbf{J}^{\top} \mathbf{J} = \mathbf{V} \mathbf{\Lambda} \mathbf{V}^{\top} $ , 设 $\lambda_i$ 为特征值,  $ \mathbf{V}=(v_1, v_2, \cdots , v_n) $ , 其中 $v_i$ 是列向量 , 有如下性质:

$(\mathbf{J}^\top \mathbf{J}) ^{-1}= (\mathbf{V} \Lambda \mathbf{V}^{\top})^{-1} = \mathbf{V} \Lambda^{-1} \mathbf{V}^{-1}$,  其中 $[\Lambda^{-1}]_{ii} = \frac {1}{\lambda_i}$
