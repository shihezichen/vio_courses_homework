...
```
  ...
  # 	for line in lines:
    ...
    ![lambda随迭代曲线变化图](D:\Study\vio\3\images\exp_lambda.jpg  "lambda随迭代曲线变化图")
...
- #### 目标2:  将曲线函数改成 $y=ax^2+bx+c$, 请修改样例代码中残差计算,雅可比计算等函数, 完成曲线参数估计
  ...
  - 4. 输出如下, 可以看到优化后的参数和真实值差别很小, 在1.e-2级别:
    ...
    ```
- 5. 执行python脚本, lambda随迭代的变化图形如下:
![ax^2+bx+c的lambda输出变化](D:\Study\vio\3\images\mul_lambda.png 'ax^2+bx+c的lambda输出变化')
...
- #### 目标3: 如果有实现其他阻尼因子更新策略可加分(选做).
  ...
  - 3. 四种策略执行结果与Lambda随迭代变化曲线图比较如下:   
      ...
      ![四种lambda策略比较]( D:\Study\vio\3\images\all_in_one.png "四种lambda策略比较")
