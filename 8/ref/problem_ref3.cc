...
int frameNum = 0;
int solve_count = 0;
double amounttime_seconds;
double avgtime_seconds;
...
//        if (sqrt(currentChi_) < 1e-15)
    ...
    amounttime_seconds += t_solve.toc();
    std::cout<<"*********************总时间***********************\n"<<amounttime_seconds<<std::endl;
...
//    ComputeLambdaInitLM();
    ...
    while (!stop && (iter < iterations)) {
        ...
        {
//            double alpha = b_.squaredNorm() / (acc_jaco_ * b_).squaredNorm();
//            VecX h_sd = -alpha * b_;
            VecX h_sd = b_;
            std::cout<<"******************hsd.norm************\n"<<h_sd.norm()<<std::endl;
            std::cout<<"*******************h_sdTh_sd**********\n"<<h_sd.transpose()*h_sd<<std::endl;
            std::cout<<"***********sqrt(h_sdT*Hessian_*h_sd)**********\n"<<sqrt(h_sd.transpose()*Hessian_*h_sd)<<std::endl;
            double alpha = h_sd.squaredNorm() / (100.*sqrt(h_sd.transpose()*Hessian_*h_sd));
            std::cout <<"**************alpha************\n"<<alpha<<std::endl;
            h_sd = alpha * b_;
            VecX h_gn = Solve_gauss_newton();
//            std::cout <<"**************hgn************\n"<<h_gn<<std::endl;
            double beta = 0.;
            if(h_gn.norm() <= radius)
            {
...
//                delta_x_ = h_gn;
            else if(alpha * h_sd.norm() >= radius)
            {
                delta_x_ = (radius / h_sd.norm()) * h_sd;
...
//                beta = (sqrt(c*c + (b-a).squaredNorm() * (radius*radius-a.squaredNorm()))-c)/(b-a).squaredNorm();
                delta_x_ = alpha * h_sd + beta * (h_gn - alpha * h_sd);
...
//                tempChi += err_prior_.squaredNorm();
            ...
            if(h_gn.norm() <= radius)
                ...
                scale = radius * (2*alpha*b_.norm() - radius) / (2. * alpha);
            else
            {
                VecX a = alpha * h_sd;
                VecX b = h_gn;
                ...
                if(c <= 0)
                    ...
                    beta = (radius*radius - a.squaredNorm()) / (sqrt(c*c + (b-a).squaredNorm() * (radius*radius-a.squaredNorm()))+c);
//                beta = (sqrt(c*c + (b-a).squaredNorm() * (radius*radius-a.squaredNorm()))-c)/(b-a).squaredNorm();
                scale = 0.5*alpha*pow((1 - beta),2)*b_.squaredNorm() + beta*(2.0-beta)*currentChi_;
            }
            scale += 1e-6;
...
//                UpdateStates();
            ...
            if(rho > 0.75)
            {
                radius = max(radius, 3.0 * delta_x_.norm());
            }
            else if(rho < 0.25)
            {
                radius = radius / 2.0;
...
//            acc_jaco_ += jacobian_i;
            ...
            //****************************************************************************
//            residuals_ += edge.second->Residual();
            //*****************************************************************************
        }
...
//*************************************************************************
VecX Problem::Solve_gauss_newton() {
    if (problemType_ == ProblemType::GENERIC_PROBLEM) {
        // PCG solver
        MatXX H = Hessian_;
        // delta_x_ = PCGSolver(H, b_, H.rows() * 2);
...
//        TicToc t_Hmminv;
        ...
        // TicToc t_linearsolver;
        VecX h_gn(VecX::Zero(delta_x_.size()));
        delta_x_pp =  H_pp_schur_.ldlt().solve(b_pp_schur_);//  SVec.asDiagonal() * svd.matrixV() * Ub;
