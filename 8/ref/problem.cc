...
bool Problem::IsGoodStepDL()
{
    ...
    if(h_dl_ == h_gn_)
        scale = currentChi_;
    else if(h_dl_ == radius_ * b_ / b_.squaredNorm())
        scale = radius_ * (2 * (alpha_ * b_).squaredNorm() - radius_) / (2 * alpha_);
    else 
        scale = 1/2 * alpha_ * pow( (1 - beta_), 2) * b_.squaredNorm() + beta_ * (2 - beta_) * currentChi_;
...
bool Problem::DLSolve(int iterations)
{
    ...
    while (!stop && (iter < iterations)) {
        ...
        while (!oneStepSuccess && false_cnt < 10)  // 不断尝试 Lambda, 直到成功迭代一步
        {
            ...
            // 计算alpha 和 h_gn 
            alpha_ = b_.squaredNorm() / (Hessian_ * b_).squaredNorm();  // 此处是H矩阵吗？？
            h_sd_ = alpha_ * b_;
            // h_sd_ = b_;
...
bool Problem::Solve(int iterations) {
    ...
    SetOrdering();
    // 遍历edge, 构建 H 矩阵
    MakeHessian(); // 每次优化都要重新将delta_x_置零

    ////////////// TODO-homework::修改为DogLeg解法
    // LM 初始化
    ComputeLambdaInitLM();
    // LM 算法迭代求解
    bool stop = false;
...
//            }
            ...
            // 更新状态量
            UpdateStates(); // 传入delta_x_更新状态量prior和vertex
...
//    scale += 1e-3;    // make sure it's non-zero :)
    ...
    // WHY修改---------------------------------------
    /// 尝试第一种更新策略（LM论文中），将lambda多乘一个主对角元素
    double maxDiagonal = 0;
    ulong size = Hessian_.cols();
    assert(Hessian_.rows() == Hessian_.cols() && "Hessian is not square");
    for (ulong i = 0; i < size; ++i) {
